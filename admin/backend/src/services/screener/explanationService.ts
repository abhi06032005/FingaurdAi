...
Do not mention specific stock names. Keep it under 80 words.`;
  ...
  try {
    const completion = await geminiQueue((genAI) => {
      const model = genAI.getGenerativeModel({
        model: 'gemini-2.0-flash',
        systemInstruction: 'You are a financial markets analyst who explains stock screening results in plain English. Be concise, factual, and educational. Never give buy/sell recommendations.',
      });
    ...
    const explanation = completion.response.text()?.trim() ?? '';
