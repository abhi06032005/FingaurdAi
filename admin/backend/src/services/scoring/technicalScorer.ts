...
export interface ScorerResult {
  ...
  technicalRating: 'Very Weak' | 'Weak' | 'Neutral' | 'Strong' | 'Very Strong';
 ...
 */
export function calculateTechnicalScore(indicators: AllIndicatorResults, currentPrice: number): ScorerResult {
  ...
  if (trend.sma50 !== null) {
    maCount++;
    if (currentPrice > trend.sma50) maScore += 5;
  }
  if (trend.sma200 !== null) {
    maCount++;
    if (currentPrice > trend.sma200) maScore += 5;
  }
  if (trend.ema50 !== null) {
    maCount++;
    if (currentPrice > trend.ema50) maScore += 5;
  }
  if (trend.ema200 !== null) {
    maCount++;
    if (currentPrice > trend.ema200) maScore += 5;
  }
  ...
  const finalScore = round(Math.max(0, Math.min(100, totalScore)), 1);

  // Map to Rating
  let rating: 'Very Weak' | 'Weak' | 'Neutral' | 'Strong' | 'Very Strong' = 'Neutral';
  if (finalScore <= 20) {
    rating = 'Very Weak';
