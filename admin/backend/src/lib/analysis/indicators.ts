...
export function computeStochastic(highs: number[], lows: number[], closes: number[], kPeriod = 14, dPeriod = 3) {
  const k: number[] = [];
  for (let i = 0; i < closes.length; i++) {
    if (i < kPeriod - 1) {
      k.push(NaN);
...
export function computeADX(highs: number[], lows: number[], closes: number[], period = 14) {
  const plusDI: number[] = [];
  const minusDI: number[] = [];
  const adx: number[] = [];

  if (closes.length === 0) return { plusDI: [], minusDI: [], adx: [] };
  ...
  const str: number[] = [];
  const sPlusDM: number[] = [];
  const sMinusDM: number[] = [];
...
export function computeVWAP(highs: number[], lows: number[], closes: number[], volumes: number[], period = 20) {
  const vwap: number[] = [];
  const vwapDeviation: number[] = [];
...
export function safeAt(arr: number[], idx: number, fallback = 0): number {
...
export function computeMACD(
  ...
  if (firstValidIdx < 0) return { macd, signal, histogram };
