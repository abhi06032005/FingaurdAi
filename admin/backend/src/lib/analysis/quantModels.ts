import { computeSMA, computeEMA, computeMACD, computeRSI, computeBollingerBands, computeATR, computeStochastic, computeWilliamsR, computeCCI, computeOBV, computeADX, computeVWAP } from './indicators';

export function calculateHurstExponent(closes: number[]): number {
  if (closes.length < 80) return 0.5;
  ...
  const computeRS = (N: number): number => {
  ...
  if (rs20 === 0 || rs40 === 0 || rs80 === 0) return 0.5;
...
export function calculateZScore(closes: number[], period = 20): number {
...
export function calculateAutocorrelation(closes: number[], lags = [1, 2, 3, 5, 10]): Record<number, number> {
  ...
  if (returns.length < 2) {
    ...
    for (const lag of lags) results[lag] = 0;
    return results;
  }
...
export function calculateDrawdown(closes: number[]) {
  if (closes.length === 0) {
...
export function calculateReturnStats(closes: number[]) {
  if (closes.length < 2) {
...
export function calculatePricePercentileRank(closes: number[]): number {
...
export function calculateVolumeProfile(closes: number[], volumes: number[]) {
...
export function computeIchimoku(highs: number[], lows: number[], closes: number[]) {
  const tenkan: number[] = [];
  const kijun: number[] = [];
...
export function calculatePivots(candles: { date: Date, open: number, high: number, low: number, close: number }[]) {
  if (candles.length === 0) {
  ...
  const grouped: Record<string, typeof candles> = {};
...
function computePivotsFromCandles(weekCandles: { open: number, high: number, low: number, close: number }[]) {
...
export function calculateConfluence(
  ...
  // Ichimoku
  trendCount += 1;
  if (ichimoku.position === "above") trendScoreSum += 1;
  else if (ichimoku.position === "below") trendScoreSum -= 1;
  ...
  volCount += 1;
  if (lastPercentB > 0.8 || lastPercentB < 0.2) volScoreSum += 1; // expanding/extreme
  else volScoreSum -= 0.5; // compressed
