...
exports.recalculateAnalysis = recalculateAnalysis;
const prisma_1 = __importDefault(require("../../config/prisma"));
const indicatorOrchestrator_1 = require("../indicators/indicatorOrchestrator");
const signalEngine_1 = require("../signals/signalEngine");
const technicalScorer_1 = require("./technicalScorer");
const confluenceEngine_1 = require("./confluenceEngine");
const scoreHistoryTracker_1 = require("./scoreHistoryTracker");
const groqSummary_1 = require("../ai/groqSummary");
const analysisCache_1 = require("../cache/analysisCache");
const nifty50Symbols_1 = require("../dataIngestion/nifty50Symbols");
 ...
 */
async function recalculateAnalysis(symbol, skipAi = false) {
    const startTime = Date.now();
    logger_1.logger.info(`Starting technical analysis recalculation for ${symbol}`);
    try {
        // 1. Fetch last 400 daily candles sorted descending (newest first)
        const dbCandles = await prisma_1.default.candle.findMany({
            where: {
                symbol,
                dataQuality: { gte: 60 }
            },
        ...
        const indicators = (0, indicatorOrchestrator_1.calculateAllIndicators)(formattedCandles);
        // 3. Generate Signals
        const signals = (0, signalEngine_1.generateSignals)(indicators, currentPrice);
        // 4. Calculate Weighted Score & Rating
        const { technicalScore, technicalRating, scoreBreakdown } = (0, technicalScorer_1.calculateTechnicalScore)(indicators, currentPrice);
        // 5. Calculate Confluence Score
        const confluenceScore = (0, confluenceEngine_1.calculateConfluence)(signals);
        // 6. Record Score History in DB (Prisma)
        await (0, scoreHistoryTracker_1.recordScoreHistory)(symbol, lastCandle.date, technicalScore, technicalRating, confluenceScore);
        // 7. Retrieve last 30 days of Score History
        const historyList = await (0, scoreHistoryTracker_1.getScoreHistory)(symbol, 30);
        ...
        try {
            const existingCache = await prisma_1.default.technicalAnalysisCache.findUnique({
                where: { symbol }
            });
            if (existingCache) {
                const result = existingCache.resultJson;
                ...
                if (cacheCandleDateStr === currentCandleDateStr && result && result.aiSummary) {
                    ...
                    logger_1.logger.info(`Reused AI summary from PostgreSQL cache for ${symbol}`);
                }
            }
        }
        catch (cacheReadError) {
            logger_1.logger.error('Failed to read PostgreSQL cache for existing AI summary', cacheReadError);
        }
        // Try MongoDB cache if not found in Postgres cache (wrapped in try-catch to allow failure bypass)
        if (!aiSummary) {
            try {
                const cachedReport = await AiReport_1.default.findOne({ symbol, candleDate: dateOnly });
                if (cachedReport) {
                ...
                logger_1.logger.warn(`Failed querying MongoDB for cached AI report on ${symbol}, proceeding with background generation:`, mongoError);
            }
        }
        // If still null, run in background
        if (!aiSummary && !skipAi) {
            // Run in background
            (0, groqSummary_1.getEducationalSummary)({
                symbol,
                ...
                if (res) {
                    logger_1.logger.info(`Background AI summary generated for ${symbol}`);
                    try {
                        const currentCache = await (0, analysisCache_1.getCachedAnalysis)(symbol);
                        if (currentCache) {
                            currentCache.aiSummary = res.summary;
                            currentCache.aiGeneratedAt = res.generatedAt.toISOString();
                            await (0, analysisCache_1.setCachedAnalysis)(symbol, currentCache, currentCache.technicalScore, lastCandle.date);
                            logger_1.logger.info(`Successfully updated cache with AI summary for ${symbol}`);
                        }
                    }
                    catch (cacheErr) {
                        logger_1.logger.error(`Failed to update cache with AI summary for ${symbol}`, cacheErr);
                    }
                }
            }).catch((err) => {
                logger_1.logger.error(`Failed background AI summary for ${symbol}`, err);
            });
        }
        // 9. Formulate final API analysis response
        const finalResponse = {
            symbol,
            companyName: (0, nifty50Symbols_1.getStockName)(symbol),
            currentPrice,
            priceChange,
            priceChangePct,
            technicalScore,
            ...
            candlesUsed,
            generatedAt: (0, dateUtils_1.formatKolkataDate)(new Date(), "yyyy-MM-dd'T'HH:mm:ssXXX"),
            isCached: false
        };
        // 10. Write to caches
        await (0, analysisCache_1.setCachedAnalysis)(symbol, finalResponse, technicalScore, lastCandle.date);
        const durationMs = Date.now() - startTime;
        logger_1.logger.info(`Successfully completed recalculation for ${symbol}`, { symbol, durationMs });
        return finalResponse;
    }
    catch (error) {
        const errMessage = error instanceof Error ? error.message : String(error);
        logger_1.logger.error(`Recalculation failed for ${symbol}: ${errMessage}`, { symbol, error });
        throw error;
    }
}
