...
exports.gatherCompanyData = gatherCompanyData;
 ...
 */
function extractMetricTable(rows, wantedMetrics, lastNPeriods) {
    ...
    const usablePeriods = periods.filter((p) => filteredRows.some((r) => r[p] !== null && r[p] !== undefined));
 ...
 */
function latestValue(rows, metricName) {
 ...
 */
function extractShareholding(shareholding, lastNPeriods = 6) {
    const quarterly = shareholding?.quarterly;
    ...
    for (const row of quarterly) {
        ...
        if (!name || name.toLowerCase().includes('shareholders'))
            continue;
 ...
 */
function extractGrowthRates(pnl) {
    const salesGrowth = pnl?.['Compounded Sales Growth'];
    const profitGrowth = pnl?.['Compounded Profit Growth'];
    const stockPrice = pnl?.['Stock Price CAGR'];
    const roe = pnl?.['Return on Equity'];
 ...
 */
function extractPeers(peers) {
    if (!Array.isArray(peers))
        return 'No data';
    // The peers array typically has [{}, {median_summary: [...]}] or actual peer rows
    const peerRows = peers.filter((p) => p && typeof p === 'object' && !p.median_summary && Object.keys(p).length > 2);
    if (peerRows.length === 0) {
        // Try to extract median summary at minimum
        const median = peers.find((p) => p?.median_summary);
    ...
    const rows = top.map((p) => keys.map((k) => String(p[k] ?? '-')).join(' | '));
    return [header, ...rows].join('\n');
}
/**
 * Extract only the P&L annual_data array from the profit_and_loss object.
 */
function getPnLRows(pnl) {
    if (Array.isArray(pnl))
        return pnl; // fallback if stored as plain array
    if (Array.isArray(pnl?.annual_data))
        return pnl.annual_data;
    return [];
}
async function gatherCompanyData(ticker) {
    ...
    const [stockDoc, annualReportDocs] = await Promise.all([
        ...
        AnnualReport_1.AnnualReport.find({ symbol })
            .sort({ fiscal_year: -1 })
            .limit(2)
    ...
    const companyName = stockDoc?.company_name ||
        annualReportDocs[0]?.company_name ||
        symbol;
...
function topLine(arr, n = 3) {
    const t = top(arr, n);
    return t.length ? t.join('; ') : 'N/A';
}
function buildAnalysisPrompt(bundle) {
    ...
    if (stock) {
        ...
        const bsRows = Array.isArray(s.balance_sheet) ? s.balance_sheet : [];
        const cfRows = Array.isArray(s.cash_flow) ? s.cash_flow : [];
        const ratioRows = Array.isArray(s.ratios) ? s.ratios : [];
        const qtrRows = Array.isArray(s.quarters) ? s.quarters : [];
        ...
        parts.push(extractMetricTable(bsRows, ['Equity Capital', 'Reserves', 'Borrowings', 'Total Liabilities', 'Fixed Assets', 'CWIP', 'Investments', 'Total Assets'], 6));
        // ── Cash Flow: last 6 years ───────────────────────────────────────────
        parts.push(`\n### Cash Flow (last 6 years, ₹ Cr)`);
        parts.push(extractMetricTable(cfRows, ['Cash from Operating Activity', 'Cash from Investing Activity', 'Cash from Financing Activity', 'Net Cash Flow', 'Free Cash Flow', 'CFO/OP'], 6));
        // ── Ratios: last 6 years ─────────────────
        parts.push(`\n### Efficiency & Return Ratios (last 6 years)`);
        parts.push(extractMetricTable(ratioRows, ['Debtor Days', 'Inventory Days', 'Days Payable', 'Cash Conversion Cycle', 'Working Capital Days', 'ROCE %'], 6));
...
async function generateAndStoreAIReport(ticker, forceRegenerate = false) {
    ...
    // ── Cache check (72h TTL) ────────────────────────────────────────────────
    if (!forceRegenerate) {
        const existing = (await CompanyAIReport_1.CompanyAIReport.findOne({ ticker: symbol }).lean());
    ...
    try {
        ...
        await CompanyAIReport_1.CompanyAIReport.findOneAndUpdate({ ticker: symbol }, {
            ...
            data_sources: {
                ...
                fiscal_years_covered: bundle.annualReports.map((r) => r.fiscal_year).filter(Boolean),
                stock_last_updated: bundle.stock?.lastUpdated,
            },
    ...
    // ── Persist ───────────────────────────────────────────────────────────────
    const saved = await CompanyAIReport_1.CompanyAIReport.findOneAndUpdate({ ticker: symbol }, {
        $set: {
            ticker: symbol,
