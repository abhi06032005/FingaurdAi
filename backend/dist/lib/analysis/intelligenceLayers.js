...
const quantModels_1 = require("./quantModels");
const clamp = (value, min = 0, max = 100) => Math.max(min, Math.min(max, Number.isFinite(value) ? value : min));
const average = (values) => values.length === 0 ? 0 : values.reduce((a, b) => a + b, 0) / values.length;
const toDateKey = (date) => date.toISOString().split('T')[0];
const scoreToStance = (score, threshold = 0.12) => {
    if (score > threshold)
        return 'supporting';
    if (score < -threshold)
        return 'conflicting';
...
const normalizeSigned = (value, divisor) => clamp(value / divisor, -1, 1);
...
function computeRegime(hurst, er, adx, rv, dd) {
    ...
    if (rv < 14 && adx < 18 && dd < 5)
        return 'Compression';
    if (adx > 22 && er > 0.38 && hurst > 0.52)
        return 'Persistent';
    if (adx < 18 && er < 0.34 && hurst <= 0.52)
        return 'Balanced';
    return 'Transitional';
}
function calculateStructureScore(close, pivots) {
    if (!pivots?.classic)
        return 0;
    const { pivot, r1, s1 } = pivots.classic;
    if (![pivot, r1, s1].every(Number.isFinite))
        return 0;
    if (close >= pivot) {
...
function buildReliabilityRankings(input) {
    ...
    const item = (model, base, requiredDays, sensitivity = 1, note) => {
...
function buildAgreementGroup(name, components) {
...
function maxForwardDrawdown(closes, startIdx, endIdx) {
...
function calculateUpgradedAnalytics(input) {
    ...
    const historicalHurst = closes.map((_, i) => (i < 80 ? 0.5 : (0, quantModels_1.calculateHurstExponent)(closes.slice(i - 80, i + 1))));
    const historicalER = closes.map((_, i) => (i < 20 ? 0.5 : (0, quantModels_1.calculateEfficiencyRatio)(closes.slice(i - 20, i + 1), 20)));
    const historicalRV = closes.map((_, i) => (i < 20 ? 0 : (0, quantModels_1.calculateRealizedVolatility)(closes.slice(i - 20, i + 1), 20)));
    const historicalZScore = closes.map((_, i) => (i < 20 ? 0 : (0, quantModels_1.calculateZScore)(closes.slice(i - 19, i + 1), 20)));
    const historicalVolumeRatio = closes.map((_, i) => {
        const avgVol = volSma20[i] || volumes[i] || 1;
        return avgVol === 0 ? 0 : volumes[i] / avgVol;
    });
    const regimes = closes.map((_, i) => computeRegime(historicalHurst[i], historicalER[i], adx[i] || 0, historicalRV[i], drawdown.underwaterCurve[i] || 0));
    ...
    const compositeProxyForIndex = (idx) => {
        if (idx < 0)
            return 0;
    ...
    const volumePressure = Math.round(clamp(50 +
        normalizeSigned(volumeRatio - 1, 1.2) * 24 +
        scoreToStance(obvSlope, 0) === 'supporting' ? 14 : -8 +
        normalizeSigned(vwapDistancePct, 4) * 12, 0, 100));
    ...
    const confidenceScoreVal = Math.round((overallAgreement * 0.3) +
        (stabilityFactor * 0.2) +
