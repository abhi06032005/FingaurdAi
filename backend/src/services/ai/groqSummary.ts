...
export interface GroqSummaryParams {
  symbol: string;
  candleDate: Date;
  currentPrice: number;
...
export async function getEducationalSummary({
  ...
  currentPrice,
  indicators
}: GroqSummaryParams): Promise<{ summary: string; generatedAt: Date } | null> {
...
Please provide an educational technical summary.`;
    ...
    logger.error(`Error in getEducationalSummary for ${symbol}:`, error);
    return null;
  }
}
