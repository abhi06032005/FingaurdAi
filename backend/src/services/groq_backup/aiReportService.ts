...
export function buildAnalysisPrompt(bundle: ReportDataBundle): string {
  ...
  for (const report of annualReports) {
    ...
    if (bd) {
      ...
      if (adv !== 'N/A') parts.push(`Moats: ${adv}`);
      const segs = topLine(bd.segments, 4);
      if (segs !== 'N/A') parts.push(`Segments: ${segs}`);
    }
    ...
    if (ceo?.summary) {
      ...
      if (pri.length) parts.push(`Priorities: ${pri.join('; ')}`);
    }
    ...
    if (md) {
      ...
      if (md.profit_explanation) parts.push(`Profit: ${md.profit_explanation}`);
      const gd = topLine(md.growth_drivers, 3);
      if (gd !== 'N/A') parts.push(`Growth drivers: ${gd}`);
      const ch = topLine(md.challenges, 2);
      if (ch !== 'N/A') parts.push(`Challenges: ${ch}`);
      const fs = topLine(md.future_strategy, 2);
      if (fs !== 'N/A') parts.push(`Strategy: ${fs}`);
    }
    ...
    if (report === annualReports[0]) {
      ...
      if (io) {
        const parts2: string[] = [];
        if (io.market_size) parts2.push(`Market: ${io.market_size}`);
        if (io.growth_rate) parts2.push(`Growth: ${io.growth_rate}`);
        if (parts2.length) parts.push(`\nIndustry — ${parts2.join(' | ')}`);
        const tw = topLine(io.tailwinds, 2);
        if (tw !== 'N/A') parts.push(`Tailwinds: ${tw}`);
        const hw = topLine(io.headwinds, 2);
        if (hw !== 'N/A') parts.push(`Headwinds: ${hw}`);
      }
      ...
      if (g?.management_outlook) {
        ...
        if (g.revenue_guidance) parts.push(`Revenue target: ${g.revenue_guidance}`);
        if (g.margin_guidance) parts.push(`Margin target: ${g.margin_guidance}`);
        if (g.confidence_level) parts.push(`Confidence: ${g.confidence_level}`);
      }

      // CapEx (top 3 only)
      if (Array.isArray(r.capex_plans) && r.capex_plans.length > 0) {
        parts.push(`\nCapEx (${r.total_capex_guidance || 'guidance N/A'}):`);
        r.capex_plans.slice(0, 3).forEach((cp: any) => {
    ...
    const risks = (r.risk_factors || [])
      .filter((rf: any) => rf.impact === 'High' || rf.impact === 'Medium' || rf.impact === 'high' || rf.impact === 'medium')
      .slice(0, 4);
    if (risks.length > 0) {
      parts.push(`\nKey Risks:`);
      risks.forEach((rf: any) => {
    ...
    if (Array.isArray(metrics) && metrics.length > 0) {
      parts.push(`\nOperational KPIs:`);
      metrics.slice(0, 4).forEach((m: any) => {
    ...
    if (ai?.bull_points?.length || ai?.bear_points?.length) {
      parts.push(`\nPrior AI context:`);
      if (ai.one_liner) parts.push(`  Summary: ${ai.one_liner}`);
      const bulls = top(ai.bull_points, 2);
      if (bulls.length) parts.push(`  Bulls: ${bulls.join('; ')}`);
      const bears = top(ai.bear_points, 2);
      if (bears.length) parts.push(`  Bears: ${bears.join('; ')}`);
    }
  }
...
async function callGroqForReport(prompt: string): Promise<{ parsed: any; raw: string }> {
  const modelLimits: Record<string, { tpm: number; defaultMaxOut: number }> = {
  ...
  for (const model of [PRIMARY_MODEL, FALLBACK_MODEL]) {
    try {
      ...
      if (err.error) console.error(`[AIReport] Groq error body:`, JSON.stringify(err.error));
    }
  }
