...
export async function generateGroqSummary(input: GroqSummaryInput): Promise<string> {
  try {
...
Write a 130–150 word plain-language market context summary following the system instructions exactly.`;
    ...
    for (const [word, replacement] of Object.entries(replacements)) {
      ...
      if (regex.test(summaryText)) {
        logger.warn(`Detected forbidden term "${word}" in Groq response. Replacing...`);
    ...
    logger.error(`Failed to generate Groq narrative summary for ${input.ticker}:`, error);
