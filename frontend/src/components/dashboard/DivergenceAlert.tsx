...
export default function DivergenceAlert({ divergences }: DivergenceProps) {
  ...
  const getDivergenceBadgeColor = (strength: string) => {
    if (strength === 'strong') return 'bg-rose-500/10 text-rose-400 border-rose-500/20';
    if (strength === 'moderate') return 'bg-amber-500/10 text-amber-400 border-amber-500/20';
    ...
    <div className="w-full flex flex-col gap-3">
      {divergences.map((div, idx) => {
        ...
        return (
          <div
            key={idx}
            className={`border rounded-xl p-4 flex gap-3 shadow-lg ${
              isBullish
          ...
          >
            <div className="mt-0.5 flex-shrink-0">
              {isBullish ? <Info size={18} className="text-sky-400" /> : <AlertTriangle size={18} className="text-amber-400" />}
            </div>
